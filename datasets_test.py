    ...
    if is_linux:
        ...
        config = {
            ...
            'CLASSIC': {
                ...
                'test_list': None,
                'train_list': None,
                'data_dir': os.path.join('data', 'Test Datasets'),
                'yita': 0.5
            },
            'DCD': {
                'img_height': 352, #240
...
class TestDataset(Dataset):
    def __init__(self,
        ...
        self.real_synthetic_bool = real_synthetic_bool
        self.mixed_bool = mixed_bool
    ...
    def _build_index(self):
        ...
        #    images_path = os.listdir(os.path.join(self.data_root))#'RGB'))
        #    labels_path = None
        #    sample_indices = [images_path, labels_path]
        sample_indices = []
        if self.test_data == "CLASSIC":
            if self.real_synthetic_bool: #True = Synthetic Dataset.
                if self.mixed_bool:
                    image_dirs = os.path.join(self.data_root, self.dataset_path, 'test', 'rgbr', 'real')
                    for directory in os.listdir(image_dirs):
                        subdir_path = os.path.join(image_dirs, directory)
                        subdir_images = os.listdir(subdir_path)
                        for filename in subdir_images:
                    ...
                    for filename in os.listdir(image_dirs):
                        sample_indices.append(
                                (os.path.join(image_dirs, filename),
                                 None,)
                        )
            else:
                '''image_dirs = os.path.join(self.data_root, self.dataset_path)
                for filename in os.listdir(image_dirs):
                    sample_indices.append(
                                (os.path.join(image_dirs, filename),
                                 None,)
                        )'''
                images_path = os.listdir(os.path.join(self.data_root, self.dataset_path))
                labels_path = None
                sample_indices = [images_path, labels_path]
        return sample_indices

    def __len__(self):
        return len(self.data_index[0]) if self.real_synthetic_bool == False else len(self.data_index) #if self.test_data.upper()=='CLASSIC'
    ...
    def __getitem__(self, idx):
        ...
        if self.real_synthetic_bool:
            if self.mixed_bool:
                ...
                head_path = head_tail[0]
                
                #print(head_path)
                #Extracting the name of the dataset to append to the image name.
                head_tail_2 = os.path.split(head_path)
                
                
                dataset_name = head_tail_2[1]
                
                
                img_name = head_tail[1]
                
                file_name = dataset_name + os.path.splitext(img_name)[0] + '.png'
                
                image = cv2.imread(image_path,cv2.IMREAD_COLOR)#'RGB',image_path), cv2.IMREAD_COLOR)
            else:
                if self.data_index[idx][1] is None:
                    image_path = self.data_index[idx][0]
                else:
            ...
            image = cv2.imread(os.path.join(self.data_root, self.dataset_path,image_path),cv2.IMREAD_COLOR)
