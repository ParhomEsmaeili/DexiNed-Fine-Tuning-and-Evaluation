...
def parse_args():
    ...
    parser.add_argument('--output_dir',
                        type = str,
                        default = 'TC_metric_output',
                        help= 'Subdirectory which contains the results, e.g. pkl files or histograms.')
    parser.add_argument('--masked_result_output_dir',
                        type=str,
                        default='IOU_masked_results',
...
def masking_artefacts(image_path_1, image_path_2, mask_path, masked_img_dir, binarised_masked_img_dir):
    ...
    masked_image_1 = (image_1_inverted_grayscale * mask).astype(np.uint8)
    masked_image_2 = (image_2_inverted_grayscale * mask).astype(np.uint8) 
    ...
    #cv2.imshow('masked_img1', masked_image_1)
    #cv2.imshow('masked_imgcorrected', binarised*mask/255)
    #cv2.imshow('masked_img2', masked_image_2)
    #cv2.waitKey(0)
    ...
    cv2.imwrite(os.path.join(masked_img_dir, image_name_1), masked_image_1)
    cv2.imwrite(os.path.join(masked_img_dir, image_name_2), masked_image_2)
    ...
    return masked_image_1, masked_image_2, mask                          #binarised_masked_image_1, binarised_masked_image_2,  

def align_images(image_1, image_2, number_of_levels, warp_mode, warp, criteria):
    ...
    for level in range(number_of_levels):
        ...
        # scale up for the next pyramid level
            tng = np.array([[1, 1, 2], [1, 1, 2]], np.float32)
            warp = warp * tng #tng.astype(np.float32)) 

    

    if warp_mode == cv2.MOTION_HOMOGRAPHY :
            # Use warpPerspective for Homography 
            image_2_aligned = cv2.warpPerspective (image_2_rescaled, warp, (sz[1],sz[0]), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP)
    else :
            # Use warpAffine for Translation, Euclidean and Affine
            image_2_aligned = cv2.warpAffine(image_2_rescaled, warp, (sz[1],sz[0]), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP)
    #cv2.imshow('image1', image_1)
    #cv2.imshow('image2', image_2)
    #cv2.imshow('image2_aligned', image_2_aligned)
    #cv2.waitKey(0)
    return image_2_aligned, failure_flag
...
def successive_frame_comparison(fused_edge_map_dir, frame_name_pair, image_mask_path, network_version, dataset_path, args):
    ...
    masked_images_output_dir = os.path.join(os.getcwd(), args.masked_result_output_dir, network_version, dataset_path)
    ...
    if not os.path.exists(masked_images_output_dir):
        os.makedirs(masked_images_output_dir)
    ...
    masked_image_1, masked_image_2, mask = masking_artefacts(frame_1_path, frame_2_path, image_mask_path, masked_images_output_dir, binarised_masked_images_output_dir)
    ...
    warped_img, ecc_failure_flag = align_images(masked_image_1, masked_image_2, number_of_levels, warp_mode, warp, termination_criteria)
    
    #Reapplying the mask to the warped image to remove/blackout the pixels which would be "outside of the image frame".
    masked_warped_img = (warped_img * mask).astype(np.uint8)
    ...
    if not ecc_failure_flag:
        ...
        iou_metric_class = metrics.IOU(masked_image_1, masked_warped_img, np.round(threshold * 255), True)
        iou_metric, iou_failure_flag = iou_metric_class.main()
        #print(iou_metric)
        #try: 
        ...
        if not iou_failure_flag:
            print('Image registration successful!')
            print('Intersection over the union is {}'.format(iou_metric))
        else:
            print('Insufficient or no overlap.')
    else:
        print('ECC Image registration totally failed!')

    return iou_metric, ecc_failure_flag, iou_failure_flag
...
def main(args):
    ...
    #Form a set of image pairs by pairing each frame with the frame after it? 
    ...
    for pair_num, image_pair in enumerate(image_pairs):
        ...
        iou, ecc_failure_flag, iou_failure_flag = successive_frame_comparison(fused_edge_map_dir, image_pair, mask_path, network_version, dataset_path, args)
        
        if not ecc_failure_flag and not iou_failure_flag:
            metric_vals.append(iou)
        ...
        elif iou_failure_flag:
            iou_failures += iou_failure_flag
            iou_failure_pairs.append(pair_num)

    print('ECC Convergence Failures {}'.format(ecc_failures))
    print('IOU Failures {}'.format(iou_failures))
    ...
    print('The average IOU is {}'.format(np.mean(metric_vals)))
    print('The median IOU is {}'.format(np.median(metric_vals)))
    
    with open(os.path.join(output_dir, 'iou_value_array'), 'wb') as fp:
        pickle.dump(metric_vals, fp)
    with open(os.path.join(output_dir, 'average_iou'), 'wb') as fp:
        pickle.dump(np.mean(metric_vals), fp)
    with open(os.path.join(output_dir, 'median_iou'), 'wb') as fp:
