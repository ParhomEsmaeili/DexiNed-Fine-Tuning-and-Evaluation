    ...
    if is_linux:
        ...
        config = {
            ...
            'CLASSIC': {
                ...
                'img_width': 512,
                'test_list': None,
                'train_list': 'classic_train_list.lst', #None,
...
class TestDataset(Dataset):
    ...
    def _build_index(self):
        ...
        if self.test_data == "CLASSIC":
            #
            image_dirs = os.path.join(self.data_root, self.dataset_version, 'edges', 'imgs', 'validation', 'rgbr', 'real')
            for directory in os.listdir(image_dirs):
                subdir_path = os.path.join(image_dirs, directory)
                subdir_images = os.listdir(subdir_path)
                stratified_subdir_images = subdir_images[::2]
                #print('frame_names are {}'.format(stratified_subdir_images))
                for filename in stratified_subdir_images:
                    sample_indices.append(
                        (os.path.join(subdir_path, filename),
                         None,)
                    )
            #print('sample indices are {}'.format(sample_indices))   
            #images_path = os.path.join(self.data_root, ) #os.path.join(self.data_root, self.test_data) #os.listdir(self.data_root)
            #labels_path = None
    ...
    def __len__(self):
        return len(self.data_index) #len(self.data_index[0]) if self.test_data.upper()=='CLASSIC' else len(self.data_index)

    def __getitem__(self, idx):
        # get data sample
        # image_path, label_path = self.data_index[idx]
        #print('data shape is {}'.format(len(self.data_index)))
        #print('Test set validation images index is {}'.format(idx))
        if self.data_index[idx][1] is None:
        ...
        if self.test_data.upper() == 'BIPED':
            ...
            img_dir = image_path #self.data_root
            gt_dir = None
        #else:
    ...
    def _build_index(self):
        ...
        if self.arg.train_data.lower()=='biped':
            ...
            images_path = os.path.join(data_root,
                                       'edges/imgs',    #Subfolder of the data_root folder. Presumably this will be the "data Folder"
                                       self.train_mode,
                                       self.dataset_type,
                                       self.data_type)
            ...
            for directory_name in os.listdir(images_path):
                ...
                for file_name_ext in os.listdir(image_directories):
                    ...
                    sample_indices.append(
                        (os.path.join(images_path, directory_name, file_name + '.jpg'),  #CHANGE THIS FROM JPG TO PNG, OR CONVERT THE FILE TYPE OF MY IMAGES TO JPG.
                         os.path.join(labels_path, directory_name, file_name + '.png'),)
                    )
        elif self.arg.train_data.lower()=='classic':
            ...
            for directory_name in os.listdir(images_path):
                ...
                for file_name_ext in os.listdir(image_directories):
                    ...
                    #print(os.path.join(images_path, directory_name, file_name + '.jpg'))
        else:
...
class ValidationDataset(Dataset):
    data_modes = ['train', 'test', 'validation' ]
    dataset_types = ['rgbr', ]
    data_types = ['aug', 'real' ]
    ...
    def __init__(self,
        ...
        self.dataset_type = dataset_type
        self.data_type = 'real' #'aug'  # be aware that this might change in the future
        self.img_height = img_height
        self.img_width = img_width
        self.mean_bgr = mean_bgr
        self.crop_img = crop_img
    ...
    def _build_index(self):
        assert self.data_mode in self.data_modes, self.data_mode
        assert self.dataset_type in self.dataset_types, self.dataset_type
        assert self.data_type in self.data_types, self.data_type
        ...
        if self.arg.train_data.lower()=='classic':
